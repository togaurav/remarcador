...
var IE_7 = false;
var IE_8 = false;
var LOADING_IMAGE = 'images/ajax-loader.gif';
//var LOADING_IMAGE = 'img/wait30trans.gif'; // IMAGEN ANTIGUA
...
(function ($) {
	...
	$(document).ready( function() {
		...
		$('<div class="errorDialogContainer"></div>').appendTo('body').ajaxError(function(event, request, settings){
			...
			if (request.statusText !== 'OK' && request.statusText !== '' && request.status != 200){
			
				if(inGlobalError){
	
					var ul = $('#global-error div ul')[0];
					$(ul).append('<li>' + request.statusText + '</li>');
					...
					var msg = '<div class="errorDialogContainerMsg">Estimado usuario<br><br><ul><li>' + request.statusText + '</li></ul><br>' +
					...
					if (request.status == 401) {
						$('.contenedor-principal').empty().append('<div style="overflow:hidden;"><iframe width="100%" height="100%" frameborder="0" src="no-tiene-acceso.html"></iframe></div>');
					}
				}
			}
		});
	...
	*/
	...
	/**
	 * Funcion que selecciona y deselecciona los botones de un menú, puede además recibir parámetros adicionales
	 * @author: mauro@continuum.cl
	 */
	$.menuSelected = function menuSelected(normalClassName, selectedClassName, idDefaultElement, hashLinks, params) {
		$.each ($('.' + normalClassName), 
			function (i, e){
				$('#' + idDefaultElement).removeClass(normalClassName);
				$('#' + idDefaultElement).addClass(selectedClassName);
	...
	$.ajaxMsg = function (opt) {
		...
		var m = '<div class="ajax-msg"><p align="center"><img src="' + LOADING_IMAGE + '"></p><p align="center">' + opt.msg + '</p></div>';
		var msg = $(m).dialog({
				modal: true, 
	...
	$.error = function (msg, block, callback) {
		if (block) {
			$('<div id="global-error" class="ui-widget ui-alert ajax-msg-error" style="padding-top:10px;"><div class="ui-state-error ui-corner-all"><p>'+
					(msg || 'Estimado usuario, <br><br>Actualmente el sitio no est&aacute; disponible.<br><br>Por favor contacte al personal de la ACHS<br><br>Muchas Gracias.<br><br>') +' </p></div></div>').dialog({
				title: "Error",
				modal: true,
				width: 600,
				zIndex: 10000, 
	...
	$.fn.loading = function(url, parametros, callback) {
		...
		var msg = $('<div><p align="center"><img src="' + LOADING_IMAGE + '"></p><p align="center">Cargando...</p></div>');
		
    		if (parametros && !parametros.r)
	...
	$.fn.loadWithoutMsg = function(url, parametros, callback){
    		if (parametros && !parametros.r)
		    parametros.r = Math.random();
		
		var div = $(this); 
		
		div.empty().load(url, parametros,  function(responseText, textStatus, XMLHttpRequest) {
			if (callback && callback.success && callback.failure){
				if (XMLHttpRequest.readyState === 4){
					if (XMLHttpRequest.status === 200) {
						if (callback && callback.success){
	...
	$.fn.showLoading = function(id) {
		var msg = $('<div id="' + (id ? id : '') +'"><p align="center"><img src="' + LOADING_IMAGE + '"></p><p align="center">Cargando...</p></div>');
		return $(this).after(msg);
	}
...
//		console.info(formValido);
	 ...
	 * 
	 */
	$.fn.btnCerrarImagenPrevia = function(div) {
		$(this).click(function(event) {
			$('.div-btn-cerrar-img').remove();
			div.fadeOut('slow');
		});
	}
		...
		return this.each(function () {
			...
			if (!div || !div.attr('id')) {
				var img = opt.urlImg ? ('<a id="previewImg-a-img-' + id + '" href="' + opt.urlImg + '" target="_blank" title="Presione para descargar imagen" >' + 
									   '<img id="previewImg-img-' + id + '"  src="' + opt.urlImg + '" alt="Imagen" onerror="this.src=\'img/noDisponible.jpg\';" /></a>') : '';
			...
			if(opt.fadeOut) {
				...
				el.hover(function () {
					if(!$('#btn-inferior-cerrar-img-previa-'+id).length || !$('#btn-superior-cerrar-img-previa-'+id).length){
						$('#previewImg-a-img-'+id).before('<div align="center" class="div-btn-cerrar-img" id="btn-superior-cerrar-img-previa-'+id+'"><button>Cerrar</button></div>');
						div.append('<div align="center" class="div-btn-cerrar-img" id="btn-inferior-cerrar-img-previa-'+id+'"><button>Cerrar</button></div>');
					}
					div.showOverElement(opt.idOver, opt.left, opt.top).fadeIn('slow');
					$('#btn-superior-cerrar-img-previa-'+id).btnCerrarImagenPrevia(div);
					$('#btn-inferior-cerrar-img-previa-'+id).btnCerrarImagenPrevia(div);
				});
			}
	...
	$.numeroRandom = function(min, max) {
		return Math.round((Math.random() * (Number(max) - Number(min))) + 1);
	}
	...
	$.fn.validateOnlyNumbers = function() {
		if (/^\d+$/.test($(this).val())) {
			return true
		} else {
			return false;
	...
	$.fn.addDays = function(days) {
		var fechaNumber = ($(this)[0]);
		var fechaDate = fechaNumber.toDate();
		fechaDate.setDate(fechaDate.getDate() + days);
		return fechaDate.toIntDate();
	}
